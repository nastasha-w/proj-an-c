...
class read_eagle_file():
    ...
    def get_full_dir(self, model_dir, filename, snapnum, add_data_to_model_dir=True):
        """Get filename, including full path"""
        if add_data_to_model_dir:
        ...
        if filename == 'snap':
            ...
            try:
                ...
                dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = dir.rsplit('_')
            ...
            try:
                ...
                dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = dir.rsplit('_')
                fname = dirpath + dir + "/eagle_subfind_snip_particles_%.3i_%s." % (snapnum, string[-1])
            except:
                dir = "particledata_%.3i/" % snapnum
                fname = dirpath + dir +"eagle_subfind_snip_particles_%.3i." % snapnum
        ...
        # snip subfind/FOF halo data:
            ...
            try:
                ...
                dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = dir.rsplit('_')
                fname = dirpath + dir + "/group_snip_tab_%.3i_%s." % (snapnum, string[-1])
            except:
            ...
            try:
                ...
                dir = fnmatch.filter( next(os.walk(dirpath))[1], dirname)[0]
                string = dir.rsplit('_')
                fname = dirpath + dir + "/eagle_subfind_snip_tab_%.3i_%s." % (snapnum, string[-1])
            except:
                dirname = "groups_snip_%.3i/" % snapnum
                fname = dirpath + dir + "/sub_%.3i." % (snapnum)
#                
